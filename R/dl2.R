...
dl2 <- function(x, polarity, err, iterations, degrees, nfold = 5, trace = .5) {
  ...
  # Normalize the data based on polarity
  normat <- normalization(x, polarity)

  # Compute correlation between variables
  corrgr <- corrgram(normat, order=TRUE, lower.panel=panel.shade, upper.panel=panel.pie, text.panel=panel.txt, main="Correlation between features")
